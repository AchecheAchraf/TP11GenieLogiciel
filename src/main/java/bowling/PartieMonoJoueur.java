...
public class PartieMonoJoueur {
	...
	public int score() {
		...
		for (int i = 0; i < nbTours-1; i++) {
			...
			if (tour.estSpare()) {
				...
				if (i+1 == nbTours-1 || !partie.get(i+1).estStrike()){
		...
		score += partie.get(nbTours-1).getNbQuilleTombeesLancer1() + partie.get(nbTours-1).getNbQuilleTombeesLancer2() + partie.get(nbTours-1).getNbQuilleTombeesLancer3();
		System.out.println(partie.get(nbTours-1).getNbQuilleTombeesLancer1() + partie.get(nbTours-1).getNbQuilleTombeesLancer2() + partie.get(nbTours-1).getNbQuilleTombeesLancer3());
		System.out.println("lancer 1 " + partie.get(nbTours-1).getNbQuilleTombeesLancer1());
		System.out.println("lancer 2 " + partie.get(nbTours-1).getNbQuilleTombeesLancer2());
		return score;
	}
	...
	public int numeroProchainLancer() {
		if (estTerminee()) return 0;
		else if (numTour == nbTours) return partie.get(nbTours-1).getNumCoup() + 1;
